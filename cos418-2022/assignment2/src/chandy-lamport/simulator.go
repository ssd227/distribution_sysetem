...
type Simulator struct {
	...
	serversDone  map[int]*map[string]bool
	snapDoneChan map[int]chan (bool)
}
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	// init serversDone map
	tmpServersFlag := make(map[string]bool)
	for sid := range sim.servers {
		tmpServersFlag[sid] = false
	}
	sim.serversDone[snapshotId] = &tmpServersFlag

	// init snapDoneChan
	sim.snapDoneChan[snapshotId] = make(chan bool)
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	// snapshotid  已完成在serverid上的 local state 和 chan的备份
	(*sim.serversDone[snapshotId])[serverId] = true

	if sim.isAllDone(snapshotId) {
		sim.snapDoneChan[snapshotId] <- true // block current process, wait for collectSnapshot
	}
...
func (sim *Simulator) CollectSnapshot(snapshotId int) *SnapshotState {
	...
	<-sim.snapDoneChan[snapshotId] // wait for "done signal"
	if debug {
		fmt.Printf("[sgd-log][CollectSnapshot] snap id[%v] cancel wait.\n", snapshotId)
	}

	for _, server := range sim.servers {
		lsnap := *server.snapshotStates[snapshotId]
		// merge tokens
		for k, v := range lsnap.tokens {
			snap.tokens[k] = v
		}
		// merge msg
		for _, chanmsg := range lsnap.messages {
			snap.messages = append(snap.messages, chanmsg)
		}
...
func (sim *Simulator) isAllDone(snapshotid int) bool {
	for _, done := range *sim.serversDone[snapshotid] {
		if !done {
			return false
		}
	}
